...
export default function Experience(){
  ...
  const [city,setCity]=useState<City>("西安");
  const [bootPhase,setBootPhase]=useState<"active"|"leaving"|"done">("active");
  ...
  useEffect(()=>{const leave=window.setTimeout(()=>setBootPhase("leaving"),650);const done=window.setTimeout(()=>setBootPhase("done"),980);return()=>{window.clearTimeout(leave);window.clearTimeout(done);};},[]);
  ...
  useEffect(()=>{function openProfile(){setView("profile");setPlusOpen(false);history.replaceState(null,"","#profile");scrollTo({top:0,behavior:"smooth"});}window.addEventListener("lvzhuang-go-profile",openProfile);return()=>window.removeEventListener("lvzhuang-go-profile",openProfile);},[]);
  const homeCases=allCases.filter((entry)=>entry.city===city);
  ...
  function selectHomeCity(next:City){const update=()=>setCity(next);const doc=document as Document&{startViewTransition?:(callback:()=>void)=>unknown};if(doc.startViewTransition)doc.startViewTransition(update);else update();window.setTimeout(()=>document.getElementById("home-cases")?.scrollIntoView({behavior:"smooth",block:"start"}),80);}
  ...
  return <main className="app-shell">
    {bootPhase!=="done"&&<div aria-label="旅妆地图正在准备" aria-live="polite" className={`app-loader ${bootPhase}`}><div className="loader-mark"><span>旅</span><i/><b/></div><strong>旅妆地图</strong><small>把喜欢的一套，安排成能出发的一天</small><div className="loader-line"><i/></div></div>}
    <header className="topbar"><button aria-label="返回旅妆地图首页" className="brand" onClick={()=>go("home")} type="button"><span className="brand-mark"><b>旅</b><i/></span><div><strong>旅妆地图</strong><small>景区妆造 · 真实案例</small></div></button><div className="top-status"><strong>{allCases.length}</strong><span>篇真实案例</span></div></header>
    {view==="home"&&<><section className="hero"><SmartImage alt={`${city}景区旅拍妆造`} src={homeCases[0]?.image||allCases[0]?.image||""}/><div className="hero-shade"/><div className="hero-copy"><span>REAL TRAVEL PORTRAITS</span><h1>先在这里，<br/>成为一次想象。</h1><p>看真实游客怎么妆、谁拍、花了多少；即使暂时不出发，也可以先收藏、试搭和做计划。</p><button onClick={()=>go("explore")} type="button">开始做一份出片计划 →</button></div></section><section className="home-content"><div className="section-heading"><div><span className="eyebrow">DESTINATIONS</span><h2>从哪座城开始种草</h2></div><small>点击城市，筛选下方案例</small></div><div className="city-story-grid">{cities.map((item,index)=>{const count=cityCaseCount(item);return <button aria-pressed={city===item} className={`${count?"city-story available":"city-story"} ${city===item?"active":""}`} key={item} onClick={()=>selectHomeCity(item)} type="button"><span>0{index+1}</span><strong>{item}</strong><b>{cityInfo[item].scene}</b><small>{count?`${count} 篇真实案例已收录`:cityInfo[item].note}</small></button>;})}</div><div className="play-banner"><div><span className="eyebrow">PLAY BEFORE YOU GO</span><h2>还没订票，也可以先云搭一套</h2><p>上传自己的照片，把凤冠、花钿、云肩和披帛拖到身上，看看哪一种风格会让你想真正去一次。</p></div><button onClick={()=>openStudio("diy")} type="button">打开云搭配 DIY</button></div><div className="section-heading home-case-heading" id="home-cases"><div><span className="eyebrow">REAL STORIES</span><h2>{city}真实旅拍灵感</h2></div><small>当前 {homeCases.length} 篇 · 全部 {allCases.length} 篇</small></div><CaseGrid favorites={favorites} items={homeCases} onFavorite={toggleFavorite} onSelect={openCase}/></section></>}
    ...
    <nav className={`bottom-nav ${plusOpen?"menu-open":""}`}><button className={view==="home"?"active":""} onClick={()=>go("home")} type="button"><span>⌂</span>首页</button><button className={view==="explore"?"active":""} onClick={()=>go("explore")} type="button"><span>⌖</span>探索</button><button aria-expanded={plusOpen} aria-label={plusOpen?"关闭创作菜单":"打开创作菜单"} className={plusOpen?"plus-button open":"plus-button"} onClick={()=>setPlusOpen((open)=>!open)} type="button"><span aria-hidden="true">＋</span><small>{plusOpen?"关闭":"创作"}</small></button><button className={view==="favorites"?"active":""} onClick={()=>go("favorites")} type="button"><span>♡</span>收藏</button><button className={view==="profile"?"active":""} onClick={()=>go("profile")} type="button"><span>○</span>我的</button></nav>
